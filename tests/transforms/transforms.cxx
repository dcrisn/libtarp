        ...
        vector<OUT> &expected_outputs)
{
    ...
    std::shared_ptr<tarp::transform<IN, OUT>> ma;

    switch(transform){
    case SMA:
        ma = make_shared<tarp::sma<IN, OUT>>(window_width);
        break;
    case WMA:
    ...
    if (actual_outputs.size() != expected_outputs.size()){
        error("number of actual outputs (%zu) != number of expected outputs (%zu)",
                actual_outputs.size(), expected_outputs.size());
        return TEST_FAIL;
    }

    for (size_t i = 0; i < actual_outputs.size(); ++i){
        if (dbcmp(actual_outputs[i], expected_outputs[i], 0.01)){
            error("FAIL: actual(%f) != expected(%f)", actual_outputs[i], expected_outputs[i]);
            return TEST_FAIL;
        }
    }
...
int main(int argc, char **argv){
   ...
   /*==========================
    ...
    std::initializer_list<float> inputs = {1.0, 3.1, 1235.222, 333.2222, 0.0031, 41.4, 47.1, 88.0};
    std::vector<float> outputs = {393.136, 392.8867, 402.4617, 105.4312, 44.12577};
    passed = run(test_ma<float,float>, TEST_PASS, SMA, 4, std::initializer_list<float>{}, inputs, outputs);
   ...
   /*==========================
    ...
    passed = run(test_ma<float,float>, TEST_PASS, WMA, 5, weights,
        initializer_list<float>{123.f, 11.8, 33.02, 0.008, 0.8, 0.99, 188.34, 4.18},
        outputs);
   ...
   /*==========================
    ...
    outputs = {45.0028, 16.9278, 28.922, 19.6734};
    passed = run(test_ma<float,float>, TEST_PASS, WMA, 5,
