...
ByteBuffer::ByteBuffer(const std::byte *bytes, size_t len) {
    if (!bytes) return;
    m_buff.insert(m_buff.end(), bytes, bytes + len);
}

ByteBuffer::ByteBuffer(const vector<std::byte> &bytes) : m_buff(bytes) {
}

ByteBuffer::ByteBuffer(int fd, int32_t num_bytes) {
...
void ByteBuffer::push(const std::vector<std::byte> &v) {
    m_buff.insert(m_buff.end(), v.begin(), v.end());
}

void ByteBuffer::push(const std::byte *start, size_t len) {
    assert(start);
    if (!start) return;
    m_buff.insert(m_buff.end(), start, start + len);
}

void ByteBuffer::from_fd(int fd, int32_t num_bytes) {
    if (fd < 0) return;
    ...
    while (num_bytes != 0) {
        int32_t size_increment = static_cast<int32_t>(read_buffer_sz);
...
std::byte ByteBuffer::get_byte(bool advance) {
    if (!within_bounds()) return std::byte {0};
    std::byte byte {m_buff.at(m_buff_offset)};
