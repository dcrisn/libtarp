...
public:
     ...
     * buffer_outputs=false, then each output is signalled and propagated to
     * the next-stage transform as appropriate *but* it is not kept in
     ...
     * For every
     */
    transform(bool buffer_outputs=true, bool trim_output = true)
        : m_buffer_outputs(buffer_outputs), m_trim_output(trim_output), m_next_stage(nullptr)
    {}
    ...
    void join(transform<OUT> *stage){
        m_next_stage = stage;
    }

    auto &get_signal(void){
        return m_new_output_signal;
    }

    bool has_output(void) const {
        return not m_output.empty();
    }
...
protected:
     ...
     */
    virtual void process_input(IN input, std::optional<OUT> &output, bool &output_is_discardable) = 0;

private:
    bool m_buffer_outputs;
    bool m_trim_output;
    transform<OUT> *m_next_stage;
...
// out values outside the reference_value +/- toleranc range. It does *not*
// fillter the values as a time series from the pov of their frequency/trend.
template <typename T>
class memoryless_bandpass_filter : public transform<T, T> {
public:
    memoryless_bandpass_filter(T low_cutoff, T high_cutoff, bool trim_output, bool buffer_outputs)
        : transform<T, T>(buffer_outputs, trim_output), m_low_cutoff(low_cutoff), m_high_cutoff(high_cutoff)
    {
            if (low_cutoff > high_cutoff) throw std::logic_error(
    ...
    memoryless_bandpass_filter(T reference_value, T tolerance, bool trim_output)
        : memoryless_bandpass_filter(reference_value - tolerance, reference_value + tolerance, trim_output, false)
    {
        if (std::is_unsigned<T>()){
            // a configuration with tolerance > reference_value would be nonsensical;
            // consider reference_value=2, with tolerance=3. The low cutoff would
            // underflow and wrap around for unsigned integers.
            if (tolerance < reference_value) std::logic_error(
                    "Invalid memoryless_bandpass_filter configuration "
...
private:
    virtual void process_input(T input, std::optional<T> &output, bool &discardable)
    {
...
class wma : public transform<IN, OUT>{
public:
...
template <typename IN, typename OUT>
class sma : public transform<IN, OUT>{
public:
    sma(size_t window_width)
        : transform<IN, OUT>(true, false),
          m_fastpath(false),
    ...
    virtual void process_input(IN input, std::optional<OUT> &output, bool &output_is_discardable) override
    {
        if (!m_fastpath){
            ...
            if (m_buffer.size() == m_window_width){
                std::for_each(m_buffer.begin(), m_buffer.end(), [this](IN elem){
                         m_sma += m_weight * elem;
                        });
                output = m_sma;
                output_is_discardable = false;
