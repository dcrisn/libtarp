...
#define revu16(i)                                                             \
    (((uint16_t)i & (uint16_t)0xff00) >> 8 | ((uint16_t)i & (uint16_t)0x00ff) \
                                               << 8)

/*
 * Reverse *bits* in a byte. */
#define revu8(i)                                                    \
    ((i & 128) >> 7 | (i & 1) << 7 | (i & 64) >> 5 | (i & 2) << 5 | \
     (i & 32) >> 3 | (i & 4) << 3 | (i & 16) >> 1 | (i & 8) << 1)
 ...
 */
#define get_bit(num, pos) ((num >> (pos - 1)) & ON_BIT)

#define set_bit(num, pos) (num | (((typeof(num))ON_BIT) << (pos - 1)))

#define set_bitval(num, pos, bitval)                 \
    ((num & ~(((typeof(num))ON_BIT) << (pos - 1))) | \
     (((typeof(num))bitval) << (pos - 1)))

#define clear_bit(num, pos)  (num & ~(((typeof(num))ON_BIT) << (pos - 1)))

#define toggle_bit(num, pos) (num ^ (((typeof(num))ON_BIT) << (pos - 1)))

/*
 * Get, set, clear, or toggle a number of adjacent bits.
 *
 * 'nbits' bits are operated on from pos to the right. See the comment on
 * get_bit for how bit positions are counted. A call such as get_bits(mynum, 4,
 * 2) will therefore get the bits at positions 4 and 3.
 *
 ...
 *  - (~0U << nbits) clears up <nbits> 0 bits on the far right, with all the
 * other bits being 1. Negating this again therefore gives us <nbits> 1 bits on
 * the far right with all the other bits being 0. This is our mask that we can
...
#define lpop_bit(num) \
    ((num & (ON_BIT << (width(num) - 1))) && ((num <<= 1) || ON_BIT))
 ...
 */
void dump_hex(const uint8_t *start,
              uint32_t len,
              uint16_t width,
              bool print_offsets);
...
namespace tarp::bit_utils {

static constexpr inline unsigned BITS_IN_BYTE {8};

// Count the number of bits in val that are turned on.
template<typename T>
unsigned count_on_bits(T val) {
    static_assert(std::is_integral_v<T>, "Invalid input, must be integral");
    static_assert(std::is_unsigned_v<T>, "Invalid input, must be unsigned");

    constexpr unsigned num_bits = sizeof(val) * BITS_IN_BYTE;
    unsigned num_on_bits = 0;
...
// Count the number of bits in val that are turned off.
template<typename T>
unsigned count_off_bits(T val) {
    constexpr unsigned num_bits = sizeof(val) * BITS_IN_BYTE;
    unsigned num_on_bits = count_on_bits(val);
    return num_bits - num_on_bits;
}

// Return the (0-based) position of the left-most bit in val that is turned on.
template<typename T>
std::pair<bool, unsigned> leftmost_on_bit_position(T val) {
    static_assert(std::is_integral_v<T>, "Invalid input, must be integral");
    static_assert(std::is_unsigned_v<T>, "Invalid input, must be unsigned");
...
// Return the (0-based) position of the right-most bit in val that is turned on.
template<typename T>
std::pair<bool, unsigned> rightmost_on_bit_position(T val) {
    static_assert(std::is_integral_v<T>, "Invalid input, must be integral");
    static_assert(std::is_unsigned_v<T>, "Invalid input, must be unsigned");
